...
import { setSalesItemSearchParam, setSalesTableData } from "@/store/slice/salesSlice";
...
const SalesTableDialog = (props) => {
  ...
  const { salesTableData, salesItemSearchParam } = useSelector(({ sales}) => sales)
  ...
  const filteredProductData = useMemo(() => {
    ...
    if(allProductList?.list?.length){
      data = allProductList?.list?.map((item) => {
        const isProductAvailable = salesTableData.every((data) => { return data?.product !== item?._id && item })
  ...
  const onSubmit = data => {
    ...
    if(data?.unique_id) {
        const index = storeSalesTableData?.findIndex((index) => index?.unique_id === data?.unique_id)
