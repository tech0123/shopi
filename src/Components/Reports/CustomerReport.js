...
const CustomerReport = () => {
  ...
  const fetchCustomerReportData = async (start = 1, limit = 7, search = "") => {
    const payload = {
      start: start,
      limit: limit,
      search: search?.trim(),
    ...
    <>
      <div className="table_main_Wrapper h-auto overflow-hidden">
        <div className="top_filter_wrap">
          <Row className="g-2 align-items-center text-center mx-auto justify-center">
            <Col sm={6}>
              <div className="page_title">
                <h3 className="m-0 customers_table_title">Top Customers</h3>
              </div>
            </Col>
          </Row>
        </div>
        <div className="table_wrapper max_height customer_table">
          <DataTable value={[]} sortField="price" sortOrder={1} rows={10}>
            <Column
              field="employee_name"
